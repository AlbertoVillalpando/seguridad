## Descripción
How about we take you on an adventure on exploring certificate signing requests Take a look at this CSR file [here](https://artifacts.picoctf.net/c/424/readmycert.csr).

## Solución
Tomamos el texto que aparece cuando leemos el archivo y le aplicamos base64, en un punto de la cadena nos aparece la bandera.

picoCTF{read_mycert_5aeb0d4f}

```bash
alberto@Nova:~/Descargas$ cat readmycert.csr 
-----BEGIN CERTIFICATE REQUEST-----

-----END CERTIFICATE REQUEST-----
```

## Notas Adicionales
|comando|descripcion|
|---|---|
|xx|xx|

## Referencias
- []()
